...
from pytracking.utils.vot_utils import calculate_failures, calculate_accuracy, calculate_expected_overlap
from pytracking.benchmarks.ar_benchmark import load_tracker
...
class EAOBenchmark:
    ...
    Args:
        dataset:
    """
    def __init__(self, dataset, tracker_name, tracker_params, skipping=5, tags=['all']):
                ...
                if len(failures) > 0:
                    ...
                    for i in range(len(points)):
                        ...
                        if i != len(points) - 1:
                            ...
                            tag_value = dataset[name].select_tag(tag, points[i], points[i+1]+1)
                            ...
                            tag_value = dataset[name].select_tag(tag, points[i], len(overlaps))
                    ...
                    tag_value = dataset[name].select_tag(tag, 0, max_idx)
